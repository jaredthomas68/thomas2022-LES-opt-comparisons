...
EngFormatter = plt.matplotlib.ticker.EngFormatter
...
function layout(colors, fontsize; showfigs=false, savefigs=false, image_directory="images/", image_name="layout-", case="low-ti-opt")
    ...
    # add boundary 
    circle = plt.matplotlib.patches.Circle((les_radius, les_radius), farm_radius, fill=nothing, color=colors[1])
    ax.add_patch(circle)
...
function opt_comparison_table(;case="high-ti", tuning="sowfa-nrel", blockage=false)
    ...
    if blockage
        # load data for power ratio for blockage effect
        datalocation = "image-data/blockage/"
        blockage_base_file = "base_case_blockage_$(case)_base_res0.5.csv"
        blockage_base_data = DataFrame(CSV.File(datalocation*blockage_base_file))
        blockage_opt_file = "base_case_blockage_$(case)_opt_res0.5.csv"
        blockage_opt_data = DataFrame(CSV.File(datalocation*blockage_opt_file))
        
        # git power ratio difference
        pr_diff = blockage_opt_data.power_ratio - blockage_base_data.power_ratio
    ...
    improvementsowfa = 100.0.*(optdirpowersowfa./(1 .+ pr_diff) .- basedirpowersowfa)./basedirpowersowfa 
    # println(mean(improvementsowfa))
    # quit
    ...
    aepoptsowfa = 365.0*24.0*sum(winddf.p .* optdirpowersowfa./(1 .+ pr_diff))
    ...
    aepdiffopt = 100.0.*(aepoptsowfa./(1 .+ mean(pr_diff)) - aepoptff)/aepoptsowfa
    ...
    errordf = DataFrame(BC=100.0.*(basedirpowersowfa .- basedirpowerff)./basedirpowersowfa,
                        OC=100.0.*(optdirpowersowfa./(1 .+ pr_diff) .- optdirpowerff)./optdirpowersowfa)
    ...
    optdirpowersowfa = round.(optdirpowersowfa.*1e-6./(1 .+ pr_diff), digits=1)
...
function directional_comparison_figure(colors, fontsize; ax = nothing, showfigs=false, savefigs=false, image_directory="images/", image_name="directional-comparison", case="high-ti", tuning="sowfa-nrel", normalize=false, plottype="power", markers=nothing, blockage=false)
    ...
    optdirpowersowfa = sum.(eachcol(optsowfadf)./(1 .+ pr_diff))./normalization_factor_sowfa
    ...
    if plottype == "power"
        ...
        ax.plot(winddf.d , optdirpowersowfa, color=colors[3], marker=markers[3], linestyle="--")
        ...
        ax.plot(winddf.d , basedirpowersowfa, color=colors[2], marker=markers[2], linestyle="--")
        ...
        ax.plot(winddf.d , optdirpowerff, color=colors[4], marker=markers[4], linestyle="--")
        ...
        ax.plot(winddf.d , basedirpowerff, color=colors[1], marker=markers[1], linestyle="--")
    ...
    elseif plottype == "improvement"
        ...
        # ax.bar(winddf.d .- 5, optdirpowersowfa.*1E-6, label="SOWFA-Opt", width=10, color=colors[3])
        ax.plot(winddf.d , improvementsowfa, color=colors[3], marker=markers[3], linestyle="--")
        compound || (case=="high-ti" && ax.annotate("SOWFA", (100, 9), color=colors[3]))
        compound || (case=="low-ti" && ax.annotate("SOWFA", (100, 10), color=colors[3]))

        # ax.bar(winddf.d .- 5, basedirpowersowfa.*1E-6, label="SOWFA-Base", width=10, color=colors[2])
        ax.plot(winddf.d , improvementff, color=colors[2], marker=markers[2], linestyle="--")
    ...
    elseif plottype == "improvement-blockage"

        # improvementsowfa = (optdirpowersowfa./basedirpowersowfa .- 1.0 .- pr_diff)*100
        improvementsowfa = (optdirpowersowfa./basedirpowersowfa .- 1.0)*100
        improvementff = (optdirpowerff./basedirpowerff .- 1.0)*100
    ...
    elseif plottype == "error"
        ...
        # ax.bar(winddf.d .- 5, optdirpowersowfa.*1E-6, label="SOWFA-Opt", width=10, color=colors[3])
        ax.plot(winddf.d , errorbase, color=colors[3], marker=markers[3], linestyle="--", label="Base")
        ...
        # ax.bar(winddf.d .- 5, basedirpowersowfa.*1E-6, label="SOWFA-Base", width=10, color=colors[2])
        ax.plot(winddf.d , erroropt, color=colors[2], marker=markers[2], linestyle="--", label="Optimized")
    ...
    elseif plottype == "wakeloss" || plottype == "annualenergyloss" || plottype == "annualenergylossincrease"
        ...
        if plottype == "annualenergylossincrease"
            ax.plot(winddf.d , increasesowfa, color=colors[3], marker=markers[3], linestyle="--", label="SOWFA")
            ax.plot(winddf.d , increaseff, color=colors[2], marker=markers[2], linestyle="--", label="BP")
        else
            ax.plot(winddf.d , optwakelosssowfa, color=colors[3], marker=markers[3], linestyle="--", label="SOWFA-optimized")
            if plottype == "annualenergyloss"
            ...
            # ax.bar(winddf.d .- 5, basedirpowersowfa.*1E-6, label="SOWFA-Base", width=10, color=colors[2])
            ax.plot(winddf.d , basewakelosssowfa, color=colors[2], marker=markers[2], linestyle="--", label="SOWFA-base")
            if plottype == "annualenergyloss"
            ...
            # ax.bar(winddf.d .+ 5, optdirpowerff.*1E-6, label="BP-Opt", width=10, color=colors[4])
            ax.plot(winddf.d , optwakelossff, color=colors[4], marker=markers[4], linestyle="--", label="BP-optimized")
            if plottype == "annualenergyloss"
            ...
            # ax.bar(winddf.d .+ 5, basedirpowerff.*1E-6, label="BP-Base", width=10, color=colors[1])
            ax.plot(winddf.d , basewakelossff, color=colors[1], marker=markers[1], linestyle="--", label="BP-base")
            if plottype == "annualenergyloss"
...
function directional_comparison_figure_compound(colors, fontsize; showfigs=false, savefigs=false, image_directory="images/", image_name="directional-comparison", blockage=false)

    figsize = [8,8]
    fig, ax = plt.subplots(4, 2, figsize=figsize, sharex=true, sharey="row")
    markers = ["o", "s", "^", "*"]
    # power
    directional_comparison_figure(colors, fontsize, ax=ax[1,1], case="high-ti", tuning="sowfa-nrel", plottype="power", markers=markers, blockage=false)
    ax[1,1].annotate("SOWFA-optimized", (160, 61.6), color=colors[3])
    ax[1,1].annotate("SOWFA-base", (150, 55.55), color=colors[2])
    ax[1,1].annotate("BP-optimized", (160, 60), color=colors[4])
    ax[1,1].annotate("BP-base", (160, 52.75), color=colors[1])
    directional_comparison_figure(colors, fontsize, ax=ax[1,2], case="low-ti", tuning="sowfa-nrel", plottype="power", markers=markers, blockage=false)
    ax[1,2].annotate("SOWFA-optimized", (140, 61), color=colors[3])
    ax[1,2].annotate("SOWFA-base", (150, 55), color=colors[2])
    ax[1,2].annotate("BP-optimized", (170, 58.2), color=colors[4])
    ax[1,2].annotate("BP-base", (80, 52), color=colors[1])
        
    # Error
    directional_comparison_figure(colors, fontsize, ax=ax[2,1], case="high-ti", tuning="sowfa-nrel", plottype="error", markers=markers, blockage=false)
    ax[2,1].annotate("Base", (130, -4.5), color=colors[3])
    ax[2,1].annotate("Optimized", (130, -6.5), color=colors[2])
    directional_comparison_figure(colors, fontsize, ax=ax[2,2], case="low-ti", tuning="sowfa-nrel", plottype="error", markers=markers, blockage=false)
    ax[2,2].annotate("Base", (130, -2.2), color=colors[3])
    ax[2,2].annotate("Optimized", (130, -6), color=colors[2])

    # Improvement
    directional_comparison_figure(colors, fontsize, ax=ax[3,1], case="high-ti", tuning="sowfa-nrel", plottype="improvement", markers=markers, blockage=false)
    ax[3,1].annotate("SOWFA", (91, 9), color=colors[3])
    ax[3,1].annotate("BP", (160, 6), color=colors[2])
    directional_comparison_figure(colors, fontsize, ax=ax[3,2], case="low-ti", tuning="sowfa-nrel", plottype="improvement", markers=markers, blockage=false)
    ax[3,2].annotate("SOWFA", (106, 10.5), color=colors[3])
    ax[3,2].annotate("BP", (180, 8.4), color=colors[2])

    # Improvement blockage adjusted
    directional_comparison_figure(colors, fontsize, ax=ax[4,1], case="high-ti", tuning="sowfa-nrel", plottype="improvement-blockage", markers=markers, blockage=blockage)
    ax[4,1].annotate("SOWFA", (160, 3.0), color=colors[3])
    ax[4,1].annotate("BP", (160, 8.75), color=colors[2])
    directional_comparison_figure(colors, fontsize, ax=ax[4,2], case="low-ti", tuning="sowfa-nrel", plottype="improvement-blockage", markers=markers, blockage=blockage)
    ax[4,2].annotate("SOWFA", (180, 3.5), color=colors[3])
    ax[4,2].annotate("BP", (145, 9), color=colors[2])

    formatter0 = EngFormatter(sep="", unit="°")
    for axi in ax    
        axi.xaxis.set_major_formatter(formatter0)
    end

    # add x label 
    ax[3,1].set(xlabel="Direction", xticks=10:60:360)
    ax[3,2].set(xlabel="Direction", xticks=10:60:360)
    ...
    ax[4,1].set(ylabel="Adj. Power Improvement (%)", xticks=10:60:360, yticks=0:2:16, ylim=[0,16])
    ax[4,1].set(xticks=10:60:360, yticks=0:2:16, ylim=[0,16])

    # adjust label padding
    pad = 9.0
    ax[1,1].yaxis.labelpad = pad
    ax[2,1].yaxis.labelpad = 0
    ax[3,1].yaxis.labelpad = pad
    ax[4,1].yaxis.labelpad = pad
    ...
    # Wake Loss
    directional_comparison_figure(colors, fontsize, ax=ax[1,1], case="high-ti", tuning="sowfa-nrel", plottype="wakeloss", markers=markers, blockage=blockage)
    ...
    directional_comparison_figure(colors, fontsize, ax=ax[1,2], case="low-ti", tuning="sowfa-nrel", plottype="wakeloss", markers=markers, blockage=blockage)
    ...
    directional_comparison_figure(colors, fontsize, ax=ax[2,1], case="high-ti", tuning="sowfa-nrel", plottype="annualenergyloss", markers=markers, blockage=blockage)
    directional_comparison_figure(colors, fontsize, ax=ax[2,2], case="low-ti", tuning="sowfa-nrel",  plottype="annualenergyloss", markers=markers, blockage=blockage)

    # Change in Directional Energy Loss
    directional_comparison_figure(colors, fontsize, ax=ax[3,1], case="high-ti", tuning="sowfa-nrel", plottype="annualenergylossincrease", markers=markers, blockage=blockage)
    ax[3,1].annotate("SOWFA", (205, 1.5), color=colors[3])
    ax[3,1].annotate("BP", (160, 3.7), color=colors[2])
    directional_comparison_figure(colors, fontsize, ax=ax[3,2], case="low-ti", tuning="sowfa-nrel",  plottype="annualenergylossincrease", markers=markers, blockage=blockage)
    ax[3,2].annotate("SOWFA", (200, 2), color=colors[3])
    ax[3,2].annotate("BP", (150, 4), color=colors[2])
...
function turbine_comparison_figures(colors, fontsize; ax1=nothing, ax2=nothing, showfigs=false, savefigs=false, image_directory="images/", image_name="turbine-comparison", case="low-ti", tuning="alldirections")
    ...
    function plot_turbine_heatmap(data, winddirections, vmin, vmax, fontsize; ax=nothing, wake_count=nothing, drawcbar=true)
        ...
        rowlabels = [string(label,"°") for label in rowlabels]
...
function turbine_comparison_figures_compound(colors, fontsize; showfigs=false, savefigs=false, image_directory="images/", image_name="turbine-comparison", case="low-ti", tuning="alldirections")
    ...
    ax[1].set_ylabel("Direction", fontsize=fontsize)
    ...
    ax[2].set_ylabel("Direction", fontsize=fontsize)
...
function horns_rev_direction_verification_figure(colors, fontsize; showfigs=false, savefigs=false, image_directory="images/", image_name="horns-rev-directions")
    ...
    function each_subfig!(ax, nsamplepoints, colors)
        ...
        # format the figure
        ax.set(xlabel="Direction", ylabel="Normalized power", ylim=[0,1])
...
function turbine_layouts(colors ;ax=nothing, rotor_diameter=126.4,les_side=5000,
    ...
    if hexagons
        ...
        ay1 = (cy + ar + ry1)/2
        ax.annotate("$(Int(round(betaouter*180/pi,digits=0)))"*("°"), (ax1, ay1))
        ...
        ay2 = (cy + ar + ry2)/2
        ax.annotate("$(Int(round(betainner*180/pi,digits=0)))"*("°"), (ax2, ay2))
...
function blockage(colors; savefigs=false, showfigs=false, fontsize=10)
    ...
    println(mean(low_ti_opt_data[!, :blockage]*scale .- high_ti_base_data[!, :blockage]*scale))
    println(mean(high_ti_opt_data[!, :blockage]*scale .- high_ti_base_data[!, :blockage]*scale))
    ax.set(ylim=[4,10], xlabel="Direction", ylabel="Rotor area over outlet area (%)", xticks=round.(directions, digits=0))
    
    formatter0 = EngFormatter(sep="", unit=L"°")
    ax.xaxis.set_major_formatter(formatter0)
...
function make_images()
    ...
    # check if images directory exists and create it if necessary
    ...
    # set whether or not to show and/or save figures
    savefigs = true 
    showfigs = true

    # generate images. Comment out any of the following lines corresponding to figures you don't want to generate
    ...
    opt_comparison_table(case="high-ti", tuning="sowfa-nrel", blockage=true)
    opt_comparison_table(case="low-ti", tuning="sowfa-nrel", blockage=true)
    
    directional_comparison_figure_compound(colors, fontsize, showfigs=showfigs, savefigs=savefigs, blockage=true)
