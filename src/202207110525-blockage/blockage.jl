...
function blockage_all_directions(;case="high-ti", tuning="sowfa-nrel", opt=false, lspacing=3.0, alpha=0, resolution=10)
    ...
    # else
    layoutdir="../inputfiles/farms/startinglayouts/angle-each-circle/"
    # end
    ...
    power_ratio_base = zeros(length(wind_resource.wind_directions))
    ...
    for i = 1:length(wind_resource.wind_directions)
        ...
        power_ratio_base[i] = powerratio(wind_resource.wind_speeds[i], blockage_ratio_base[i])
    end
    ...
    power_ratio_opt = zeros(length(wind_resource.wind_directions))
    ...
    for i = 1:length(wind_resource.wind_directions)
        ...
        power_ratio_opt[i] = powerratio(wind_resource.wind_speeds[i], blockage_ratio_opt[i])
    end

    # save results 
    # save results 
    df = DataFrame(direction=wind_resource.wind_directions, blockage=blockage_ratio_base, powerratio=power_ratio_base)
    CSV.write("base_case_blockage_$(case)_base_res$resolution.csv", df)
    df = DataFrame(direction=wind_resource.wind_directions, blockage=blockage_ratio_opt, powerratio=power_ratio_opt)
